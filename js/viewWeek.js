  ...
  async renderEditor() {
    ...
    const rowsHtml = weekDates.map(d => {
      ...
      const periodCellsHtml = Array.from({ length: maxP }).map((_, i) => {
        ...
        return `<td class="editable-cell edit-class-cell" data-p="${i + 1}" contenteditable="true" style="padding:6px; border:1px solid #cbd5e1; font-size:1rem; color:#047857; background:#ecfdf5; vertical-align:top; white-space:pre-wrap; text-align:left;" oninput="window.weekViewInstance.syncScheduleInputs()">${cellText.trim()}</td>`;
      }).join('');

      // 🌟 [핵심 수정 2] 주간 에디터 테이블 구조를 년간/월간과 완벽하게 통일 (날짜/구분/내용 레이아웃)
      return `
        <tr data-week-date="${d.dateStr}">
          <td rowspan="${store.showClass ? 2 : 1}" class="${isToday ? 'week-today-cell' : ''}" style="width: 110px; padding:8px 4px; border:1px solid #cbd5e1; background:#f8fafc; vertical-align:middle; position: static !important; z-index: auto !important; transform: none !important;">
            <div style="display:flex; flex-direction:column; align-items:center; gap:4px;">
              <span onclick="window.goToDay('${d.dateStr}')" style="font-size:1.8rem; font-weight:900; color:${dateColor}; line-height:1; cursor: pointer;" title="${d.dateStr} 일 보기로 이동">${d.day}</span>
              <span style="font-size:0.95rem; font-weight:600; color:${dateNumColor}; line-height:1;">${d.dateDisplay}</span>
              ${holidayName ? `<span style="font-size:0.75rem; color:#ef4444; font-weight:bold; margin-top:2px;">${holidayName}</span>` : ''}
          ...
          <td style="width: 60px; padding:4px; border:1px solid #cbd5e1; background:#f0f9ff; color:#0369a1; font-weight:bold; font-size:0.9rem; vertical-align:middle; text-align:center; position: static !important; z-index: auto !important; transform: none !important;">
              일정<br>
          ...
          <td colspan="${maxP}" style="text-align: left; padding: 6px 10px; border:1px solid #cbd5e1; background: #f0f9ff; vertical-align:top; position: static !important; z-index: auto !important; transform: none !important;">${compactEditorHtml}</td>
        </tr>
    ...
    this.container.innerHTML = `
      <div class="table-container" style="background:#fff; padding:12px; border-radius:8px; overflow:visible;">
        <table style="width:100%; border-collapse:collapse; text-align:center;">
          <!-- 🌟 [핵심 수정 2] <th> 대신 일반 <tbody>+<td>를 사용하여 스크롤 시 머리글이 절대 고정되지 않게 회피 -->
  ...
  generateCompactEventEditor(dateStr) {
      ...
      return list.map((e, idx) => {
          ...
          const chipsHtml = labelObjs.map(lObj => {
              const chipClickAttr = isAuthor ? `onclick="window.weekViewInstance.toggleCompactEventLabel('${dateStr}', ${idx}, '${lObj.id}')"` : '';
