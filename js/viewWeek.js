// js/viewWeek.js

function renderWeekViewer(container) {
  let html = `
    <div class="clean-viewer-board">
      <table style="min-width: 800px; text-align: center; border-collapse: collapse;">
        <tbody>
  `;

  mockJulyWeekData.forEach(d => {
    const events = d.periods.map(p => p.event).filter(e => e.trim() !== '');
    const eventText = events.join(', ') || '-';

    html += `
      <tr>
        <td rowspan="3" style="width: 60px; font-weight: bold; background: #f8fafc; border: 1px solid #cbd5e1; vertical-align: middle;">
          ${d.day}<br><span style="font-size:0.75rem; color:#64748b;">${d.date}</span>
        </td>
        <td style="width: 50px; font-weight: bold; background: #eff6ff; color: #1e40af; border: 1px solid #cbd5e1; vertical-align: middle;">일정</td>
        <td colspan="6" style="text-align: left; padding: 8px 10px; font-size: 0.85rem; color: #0369a1; background: #f0f9ff; border: 1px solid #cbd5e1;">${eventText}</td>
      </tr>
      <tr>
        <td rowspan="2" style="font-weight: bold; background: #f1f5f9; color: #475569; border: 1px solid #cbd5e1; vertical-align: middle;">수업</td>
        ${[1, 2, 3, 4, 5, 6].map(p => `<td style="font-weight: bold; background: #f8fafc; color: #334155; width: 14%; border: 1px solid #cbd5e1;">${p}교시</td>`).join('')}
      </tr>
      <tr>
        ${[1, 2, 3, 4, 5, 6].map(p => {
          const pObj = d.periods.find(item => item.p === p) || {};
          return `<td style="vertical-align: top; height: 60px; text-align: left; padding: 8px; border: 1px solid #cbd5e1;">
              ${pObj.subject ? `<div style="margin-bottom: 4px;"><span class="badge-tag">${pObj.subject}</span></div>` : ''}
              ${pObj.memo ? `<div class="clean-cell-memo">${pObj.memo}</div>` : ''}
            </td>`;
        }).join('')}
      </tr>
    `;
  });
  html += `</tbody></table></div>`;
  container.innerHTML = html;
}

function renderWeekEditor(container) {
  let html = `
    <div class="table-container">
      <table style="min-width: 800px; text-align: center; border-collapse: collapse;">
        <tbody>
  `;
  mockJulyWeekData.forEach(d => {
    const events = d.periods.map(p => p.event).filter(e => e.trim() !== '');
    const eventText = events.join(', ');

    html += `
      <tr>
        <td rowspan="3" style="width: 60px; font-weight: bold; background: #f8fafc; border: 1px solid #cbd5e1; vertical-align: middle;">${d.day}<br><span style="font-size:0.75rem; color:#64748b;">${d.date}</span></td>
        <td style="width: 50px; font-weight: bold; background: #eff6ff; color: #1e40af; border: 1px solid #cbd5e1; vertical-align: middle;">일정</td>
        <td colspan="6" class="editable-cell" contenteditable="true" style="text-align: left; padding: 8px 10px; font-size: 0.85rem; color: #0369a1; background: #f0f9ff; border: 1px solid #cbd5e1;">${eventText}</td>
      </tr>
      <tr>
        <td rowspan="2" style="font-weight: bold; background: #f1f5f9; color: #475569; border: 1px solid #cbd5e1; vertical-align: middle;">수업</td>
        ${[1, 2, 3, 4, 5, 6].map(p => `<td style="font-weight: bold; background: #f8fafc; color: #334155; width: 14%; border: 1px solid #cbd5e1;">${p}교시</td>`).join('')}
      </tr>
      <tr>
        ${[1, 2, 3, 4, 5, 6].map(p => {
          const pObj = d.periods.find(item => item.p === p) || {};
          const cellContent = (pObj.subject ? `[${pObj.subject}] ` : '') + (pObj.memo || '');
          return `<td class="editable-cell" contenteditable="true" style="vertical-align: top; height: 60px; text-align: left; padding: 8px; border: 1px solid #cbd5e1; white-space: pre-wrap;">${cellContent}</td>`;
        }).join('')}
      </tr>
    `;
  });
  html += `</tbody></table></div>`;
  container.innerHTML = html;
}