...
export class WeekView extends BaseView {
  ...
  async renderViewer() {
    ...
    const schedulesMap = {};
    schedulesSnap.forEach(doc => { schedulesMap[doc.id] = doc.data().periods || {}; });

    let html = `<div class="clean-viewer-board"><table><tbody>`;
    const realTodayStr = formatDate(new Date());

    for (const d of weekDates) {
      // 🌟 [수정 완료] periods 변수는 여기서 단 한 번만 선언 및 할당됩니다.
      const periods = schedulesMap[d.dateStr] || {};
      const eData = eventsMap[d.dateStr] || {};
      ...
      if (finalEvents.length > 0) {
          ...
          eventHtml = generateEventBadgesHTML(processedEvents, d.dateStr); 
      }
      ...
      if (isRedDay(d.dateStr, processedEvents)) {
          dateColor = '#ef4444'; 
          dateNumColor = '#ef4444'; 
  ...
  async renderEditor() {
    ...
    for (const d of weekDates) {
      ...
      let dateColor = '#1e40af';
      let dateNumColor = '#475569'; 
      if (isRedDay(d.dateStr, window[`tempEvents_${d.dateStr}`])) {
          dateColor = '#ef4444';
          dateNumColor = '#ef4444'; 
      } else if (d.dayOfWeekNum === 6) {
          dateColor = '#3b82f6';
