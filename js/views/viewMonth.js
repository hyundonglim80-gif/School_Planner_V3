...
export class MonthView extends BaseView {
  ...
  async renderViewer() {
    ...
    try {
        ...
        const { eMap, sMap } = await fetchCalendarData(startStr, endStr, this.myGroups); // 🌟 신규 통합 함수 적용
    ...
    try {
        ...
        const rowsHtml = Array.from({ length: lastDate }).map((_, i) => {
            ...
            filters.forEach(fId => {
                ...
                fEvents.forEach(e => {
                    window[`tempEvents_${dateStr}`].push({ ...e, labelIds: e.labelIds || [], sharedGroupId: fId === 'personal' ? null : fId });
                });
            });
