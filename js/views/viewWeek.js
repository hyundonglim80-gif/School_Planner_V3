...
export class WeekView extends BaseView {
  ...
  async renderViewer() {
    ...
    try {
        ...
        try { this.myGroups = await dbAPI.loadMyGroups(); } catch(e) { this.myGroups = []; } // 🌟 그룹 선로드
        const { eMap, sMap } = await fetchCalendarData(weekDates[0].dateStr, weekDates[weekDates.length - 1].dateStr, this.myGroups); // 🌟 신규 통합 함수 적용
    ...
    try {
        ...
        const rowsHtml = weekDates.map(d => {
          ...
          filters.forEach(fId => {
              ...
              const fEvents = (eMap[d.dateStr]?.eventList || []).filter(e => (e.sharedGroupId || 'personal') === fId);
              fEvents.forEach(e => {
                  window[`tempEvents_${d.dateStr}`].push({ ...e, labelIds: e.labelIds || [], sharedGroupId: fId === 'personal' ? null : fId });
              });
          });
