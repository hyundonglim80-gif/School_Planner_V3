...
// 🌟 화면 리사이즈 시 헤더 높이에 맞춰 여백 자동 조절 (전역 리스너 1회 등록)
window.addEventListener('resize', () => {
    const needsFixedHeader = store.scope === 'year' || (store.scope === 'month' && store.mode === 'editor');
    if (needsFixedHeader) {
...
export const updateButtonUI = () => {
    ...
    // 🌟 [버그 수정] 월간-작성, 년간 탭에서 스크롤 시 상단 2행(헤더)이 사라지는 현상 완벽 해결
    const needsFixedHeader = store.scope === 'year' || (store.scope === 'month' && store.mode === 'editor');
