...
export class MemoView extends BaseView {
  ...
  async saveLinks(linksArray) {
    try { 
        await setDoc(doc(getUserCol('settings'), 'user_links'), { links: linksArray }); 
  ...
  async renderViewer() {
    ...
    // 🌟 [검색 연동] 특정 메모를 찾아가야 할 경우, 다른 라벨에 숨겨져 있지 않도록 무조건 '전체' 필터로 전환
  ...
  _drawHTML() {
    ...
    // 🌟 [검색 연동] 특정 메모 아이디가 전달된 경우, 렌더링이 끝나자마자 해당 위치로 스크롤 & 하이라이트!
    if (window.targetMemoId) {
        const targetId = window.targetMemoId;
        window.targetMemoId = null; // 중복 실행 방지를 위해 바로 초기화
        ...
        setTimeout(() => {
            ...
            if (targetEl) {
                ...
                // 노란색 배경으로 깜빡이게 하는 하이라이트 효과
                const originalBg = targetEl.style.backgroundColor || '';
                targetEl.style.transition = 'background-color 0.4s ease';
                targetEl.style.backgroundColor = '#fef08a'; // 노란색(tailwind yellow-200)
  ...
  generateMemoHTML(item, isCompleted) {
    ...
    // 🌟 [검색 연동] 타겟 스크롤을 위해 컨테이너에 id="memo-card-..." 속성 추가
    return `
