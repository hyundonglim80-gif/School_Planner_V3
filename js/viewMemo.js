...
export class MemoView extends BaseView {
  ...
  toggleMemoItem(firestoreId, currentStatus) {
    ...
    dbAPI.updateMemo(firestoreId, { completed: isNowCompleted, completedAt: isNowCompleted ? Date.now() : null }).catch(e=>console.warn(e));
  }
  ...
  clearCompletedMemos() {
    ...
    if(confirm(`완료된 업무 ${completedMemos.length}개를 모두 삭제하시겠습니까?\n(이 작업은 되돌릴 수 없습니다)`)) {
      ...
      this._drawHTML();
      completedMemos.forEach(memo => dbAPI.deleteMemo(memo.firestoreId).catch(e=>console.warn(e)));
    }
  }
