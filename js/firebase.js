...
console.log("🔥 SP3.6 공유 그룹 기반 오프라인-퍼스트 Firebase 엔진이 성공적으로 연결되었습니다.");
...
export const dbAPI = {
    ...
    updateMemo: async (firestoreId, updateData, groupId = null) => { 
        if (groupId) {
            await updateDoc(doc(getGroupCol(groupId, 'tasks'), firestoreId), updateData);
        } else {
    ...
    loadGroupDayData: async (dateStr, groupId) => {
        try {
            ...
            return { 
                eventText: eventDoc.exists() ? eventDoc.data().eventText : '',
                eventList: eventDoc.exists() ? (eventDoc.data().eventList || []) : [],
                periods: scheduleDoc.exists() ? (scheduleDoc.data().periods || {}) : {} 
            };
    ...
    loadMyGroups: async () => {
        ...
        try {
            ...
            console.warn("오프라인 상태이거나 그룹 목록을 불러올 수 없습니다.", e);
            return [];
        }
    },
    ...
    createGroup: async (groupName) => {
        ...
        const groupData = {
            name: groupName,
            ownerId: user.uid,
            ownerName: user.displayName || '이름 없음',
            members: [user.uid],
            memberDetails: {
                [user.uid]: { name: user.displayName || '이름 없음', joinedAt: Date.now(), photoURL: user.photoURL || '' }
            },
    ...
    leaveGroup: async (groupId) => {
        ...
        if (groupSnap.exists()) {
            if (groupSnap.data().ownerId === user.uid) {
                throw new Error("그룹장은 그룹을 탈퇴할 수 없습니다. 대신 [그룹 삭제]를 이용해주세요.");
            }
    ...
    deleteGroup: async (groupId) => {
        ...
        if (groupSnap.exists() && groupSnap.data().ownerId === user.uid) {
            ...
            throw new Error("그룹 삭제 권한이 없습니다. (그룹장만 가능)");
        }
    }
};
