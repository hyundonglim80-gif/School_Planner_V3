...
class DayView extends window.BaseView {
  ...
  // 🚀 [수정] 일정 작성 칸 왼쪽에 체크박스 배치 (레이아웃 개선)
  renderEventEntries() {
    ...
    this.currentEvents.forEach((e, index) => {
        ...
        // 텍스트와 체크박스를 감싸는 새로운 열
        const contentRow = document.createElement('div');
        contentRow.style.cssText = "display:flex; align-items:flex-start; gap:10px; width:100%;";
        
        if (canComplete) {
            const chkWrapper = document.createElement('div');
            chkWrapper.style.cssText = "padding-top: 12px;"; // 체크박스를 텍스트 첫 줄에 보기 좋게 맞춤
            chkWrapper.innerHTML = `<input type="checkbox" class="event-complete-check" ${isCompleted ? 'checked' : ''} style="width:20px; height:20px; cursor:pointer; accent-color:#059669;" title="완료 처리">`;
        ...
        if (canComplete) {
            contentRow.querySelector('.event-complete-check').addEventListener('change', () => { this.syncEventInputs(); this.renderEventEntries(); });
        }
