...
class DayView extends window.BaseView {
        ...
        if (journals.length > 0) {
            ...
            journals.forEach(j => {
                ...
                if (j.labelIds && j.labelIds.length > 0) {
                    const masterJournals = window.getJournalLabels();
    ...
    renderEventEntries() {
        ...
        const allLabelsObj = window.getEventLabels();
        ...
        this.currentEvents.forEach((ev, idx) => {
            ...
            input.oninput = () => { window.hasUnsavedChanges = true; };
    ...
    renderJournalEntries() {
        ...
        const allLabelsObj = window.getJournalLabels();
    ...
    openClassSwapModal(period) {
        ...
        for(let i=1; i<=max; i++) {
            if (i !== period) {
                const pName = window.periodNames[i-1] || `${i}교시`;
        ...
        const targetName = window.periodNames[period-1] || `${period}교시`;
        const modalHtml = `
