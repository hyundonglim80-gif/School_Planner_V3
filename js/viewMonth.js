...
export class MonthView extends BaseView {
  ...
  async renderViewer() {
    ...
    for(let i=1; i<=lastDate; i++) {
      ...
      dayPromises.push(getUserCol('events').doc(dateStr).get().then(doc => ({ 
    ...
    let scheduleMap = {};
    try {
      const snap = await getUserCol('schedules').get();
  ...
  async renderEditor() {
    ...
    for(let i=1; i<=lastDate; i++) {
      ...
      dayPromises.push(
        Promise.all([
          dbAPI.loadDayData(dateStr), 
          getUserCol('events').doc(dateStr).get()
