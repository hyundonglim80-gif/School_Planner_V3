...
export const loadSettings = async () => { 
    ...
    } catch (error) { console.warn("설정 로드 에러:", error); }
};

// ==========================================================================
// 🖥️ 2. 메인 렌더링 엔진 (동적 디스패치)
// ==========================================================================
...
export const saveCurrentViewData = (silent = false) => {
    ...
    const view = window[`${store.scope}ViewInstance`];
    let savePromise = (view && typeof view.save === 'function') ? view.save() : null;
    ...
    setTimeout(async () => {
        ...
        } catch (e) { console.error("🚨 백그라운드 저장 에러:", e); }
    }, 0);
};
...
const initApp = () => {
    ...
    // 🌟 안전장치 복구: 화면을 끄거나 벗어날 때 작성 중인 텍스트박스 내용을 즉시 메모리에 동기화!
    ...
    try {
        ...
        while (curD <= endD) {
            ...
            curList.forEach(ev => {
                ...
                if (ev.labelIds?.length > 0) { canComplete = ev.labelIds.some(id => window.isForwardLabel(id)); } 
...
// ==========================================================================
// 📅 5. 다중 일정(기간/반복) 설정 및 삭제 팝업 엔진 (🌟 복구 완료!)
// ==========================================================================
...
export const executeGroupSave = async (labelName, callback, mode, labelId) => {
    ...
    for(let i=0; i<totalDays; i++) {
        ...
        const docRef = doc(getUserCol('events'), dStr);
        ...
        listContainer.innerHTML = store.dDayList.map(dday => {
            ...
            return `
                <button class="dropdown-item" onclick="window.selectDday('${dday.id}')" style="${isSelected ? 'background:#eff6ff; color:#2563eb;' : ''}">
                    <span style="font-weight:bold; margin-right:8px; display:inline-block; width:50px;">${calculateDday(dday.date)}</span> 
                    ${dday.title}
                </button>
            `;
...
export const selectDday = async (id) => {
    ...
    if (window.auth?.currentUser) {
        await setDoc(doc(getUserCol('settings'), 'preferences'), { selectedDDayId: id }, { merge: true });
    }
};
...
export const renderDdaySettingsList = () => {
    ...
    listEl.innerHTML = store.dDayList.map(dday => `
...
export const saveDdayDataToFirebase = async () => {
    ...
    try { await setDoc(doc(getUserCol('settings'), 'preferences'), { dDayList: store.dDayList, selectedDDayId: store.selectedDDayId }, { merge: true }); } 
    catch (e) { console.error("D-Day 저장 실패", e); alert("저장에 실패했습니다."); }
};

// ==========================================================================
// 🌉 7. 과도기 호환성 레이어 (전역 바인딩 100% 완전 복구!)
// ==========================================================================
