...
export const updateButtonUI = () => {
  // 1. 범위 버튼 (메모/년간/월간/주간/하루) 활성화 색상 표시
  const scopeBtns = document.querySelectorAll('.btn-scope');
  scopeBtns.forEach(btn => {
    btn.classList.remove('active');
    // 기존의 onclick 대신 새로 부여한 data-scope를 기준으로 확인합니다.
    if (btn.getAttribute('data-scope') === store.scope) {
      btn.classList.add('active');
    }
  });

  // 2. [수정됨] 메모 페이지 선택 시 두 번째 줄(Header Row) 통째로 숨기기
  const dateRangeText = document.getElementById('date-range-text');
  if (dateRangeText) {
      const secondHeaderRow = dateRangeText.closest('.header-row');
      if (secondHeaderRow) {
          // 메모 화면일 때는 아예 두 번째 줄 전체 영역을 없앱니다.
          secondHeaderRow.style.display = (store.scope === 'memo') ? 'none' : 'flex';
      }
  }

  // 3. 주말 및 수업 숨기기 버튼 텍스트 변경
...
const initApp = () => {
  ...
  // 2. 상단 헤더 주요 기능 이벤트 연결 (신규 추가)
  const btnToggleWeekend = document.getElementById('btn-toggle-weekend');
  if (btnToggleWeekend) btnToggleWeekend.addEventListener('click', toggleWeekend);

  const btnToggleClass = document.getElementById('btn-toggle-class');
  if (btnToggleClass) btnToggleClass.addEventListener('click', toggleClass);
  ...
  // 3. 뷰 모드(Scope) 선택 버튼 그룹 이벤트 연결 (신규 추가)
  const scopeBtns = document.querySelectorAll('.btn-scope');
  scopeBtns.forEach(btn => {
  ...
  // 4. 날짜 이동 버튼 이벤트 연결 (신규 추가)
  const btnPrevDate = document.getElementById('btn-prev-date');
  const btnNextDate = document.getElementById('btn-next-date');
  const dateRangeText = document.getElementById('date-range-text');
  
  if (btnPrevDate) btnPrevDate.addEventListener('click', () => moveDate(-1));
  if (btnNextDate) btnNextDate.addEventListener('click', () => moveDate(1));
  if (dateRangeText) dateRangeText.addEventListener('click', goToToday);

  // 5. 저장되지 않은 데이터 감지 (기존 유지)
