...
export const openNativeClock = () => {
    ...
    if (userAgent.includes('win')) {
        ...
        // 안드로이드 기본 시계 앱 호출 (제조사에 따라 다를 수 있음)
        ...
        alert("애플 기기(iOS/Mac)는 보안 정책상 웹에서 기본 시계 앱을 직접 실행할 수 없습니다.\n제어센터의 타이머나 시리(Siri)를 이용해 주세요.");
    } else {
...
export const executeGroupDelete = async (mode, baseDateStr, groupId, labelIdOrName, baseContent, onConfirm) => {
    document.getElementById('group-delete-modal').innerHTML = `<div style="background:#fff; padding:30px; border-radius:12px; font-weight:bold; color:#2563eb; text-align:center;">⏳ 삭제 처리 중...</div>`;
