...
export const LabelManager = {
  ...
  saveEventLabels: async function(e) {
      ...
      if (window.auth && window.auth.currentUser) {
          try {
              await getUserCol('settings').doc('labels').set({ eventLabels: dataToSave }, { merge: true });
      ...
      if (deletedIds.length > 0 && window.db) {
          ...
          try {
              const eventsSnap = await getUserCol('events').get();
  ...
  saveJournalLabels: async function(e) {
      ...
      if (window.auth && window.auth.currentUser) {
          try { await getUserCol('settings').doc('labels').set({ journalLabels: dataToSave }, { merge: true }); } catch (err) {}
      }
      ...
      if (deletedIds.length > 0 && window.db) {
          ...
          try {
              const snap = await getUserCol('journals').get();
              let batch = window.db.batch();
              let count = 0;
              let batchPromises = [];
  ...
  saveMemoLabels: async function(e) {
    ...
    if (window.db && window.auth && window.auth.currentUser) {
        try { await getUserCol('settings').doc('labels').set({ memoLabels: dataToSave }, { merge: true }); } catch (err) {}
    }
