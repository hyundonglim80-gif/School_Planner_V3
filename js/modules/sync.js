        ...
        <div class="modal-content" style="max-width: 550px;">
            ...
            <div class="modal-body" style="max-height: 70vh; overflow-y: auto;">
                ...
                <div style="border: 2px solid #bbf7d0; border-radius: 8px; padding: 15px; background: #f0fdf4;">
                    ...
                    <button class="modal-btn-primary" style="width:100%; background:#16a34a;" onclick="window.executeGoogleImport()">📥 SP3.1로 가져오기</button>
                </div>
...
window.executeGoogleExport = async function() {
    ...
    try {
        ...
        if (syncEvent || syncClass || syncJournal) {
            ...
            const syncMetaDoc = await getUserCol('settings').doc('google_sync_meta').get();
            let syncMeta = syncMetaDoc.exists ? syncMetaDoc.data().dateSyncTimes || {} : {};

            const [eSnap, sSnap, jSnap] = await Promise.all([
                getUserCol('events').where(firebase.firestore.FieldPath.documentId(), '>=', startStr).where(firebase.firestore.FieldPath.documentId(), '<=', endStr).get(),
                getUserCol('schedules').where(firebase.firestore.FieldPath.documentId(), '>=', startStr).where(firebase.firestore.FieldPath.documentId(), '<=', endStr).get(),
                getUserCol('journals').where(firebase.firestore.FieldPath.documentId(), '>=', startStr).where(firebase.firestore.FieldPath.documentId(), '<=', endStr).get()
            ]);
            ...
            for (let i = 0; i < total; i++) {
                ...
                if (i > 0 && i % 20 === 0) {
                    await getUserCol('settings').doc('google_sync_meta').set({ dateSyncTimes: syncMeta }, { merge: true });
                }
            }
...
window.executeGoogleImport = async function() {
    ...
    try {
        ...
        updateProgress("💾 SP3.1 데이터베이스에 저장 중...", 60);
        let eventsByDate = {};
        ...
        while (curD <= endD) {
            ...
            if (newEvents.length > 0 || mode === 'replace' || mode === 'overwrite') {
                ...
                if (isSkipDay) {
                    const scheduleRef = getUserCol('schedules').doc(dStr);
            ...
            processedCount++;
            updateProgress(`💾 SP3.1 데이터베이스에 저장 중... [${processedCount}/${totalDays}]`, 60 + (40 * (processedCount/totalDays)));
            curD.setDate(curD.getDate() + 1);
        }
...
async function getOrCreateDedicatedCalendar(token) {
    ...
    let targetCal = data.items ? data.items.find(cal => cal.summary === 'School Planner V3.1') : null;
    if (targetCal) return targetCal.id;

    // 호환성을 위해 기존 'School Planner V3' 캘린더가 있는 경우 명칭 업데이트 또는 탐색 지원
    let oldCal = data.items ? data.items.find(cal => cal.summary === 'School Planner V3') : null;
    if (oldCal) return oldCal.id;

    const createUrl = "https://www.googleapis.com/calendar/v3/calendars";
    const newCal = await googleFetch(createUrl, 'POST', token, {
        summary: 'School Planner V3.1',
