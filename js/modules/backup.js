...
export const BackupManager = {
    ...
    executeExport: async function() {
        ...
        if (target === 'sheets') {
            if (options.scope !== 'personal') return alert("구글 시트 연동은 개인 데이터만 지원합니다.");
            await BackupGoogle.exportToSheets(options);
        } else if (target === 'csv') {
            await BackupLocal.handleDownload(options, scopeName);
        } else if (target === 'calendar') {
            if (options.scope !== 'personal') return alert("구글 캘린더 연동은 개인 데이터만 지원합니다.");
    ...
    handleUpload: function(input) {
        ...
        const options = this.getOptions();
        const scopeName = options.scope === 'personal' ? '개인 데이터' : '공유 그룹 데이터';
        BackupLocal.handleUpload(file, options, scopeName);
        input.value = '';
    },
