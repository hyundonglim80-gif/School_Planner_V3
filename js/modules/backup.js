...
export const BackupManager = {
    ...
    checkExistingSheet: async function() {
        try {
            const doc = await getUserCol('settings').doc('backup_config').get();
    ...
    resetSheetConnection: async function() {
        if(confirm("기존 구글 시트와의 연결을 강제로 끊고 완전히 새로운 파일을 생성하시겠습니까?\n\n(구글 드라이브에서 파일을 완전히 삭제했거나 찾을 수 없을 때 누르세요!)")) {
            await getUserCol('settings').doc('backup_config').delete();
    ...
    getScheduleDataArray: async function() {
        ...
        const eventsSnap = await getUserCol('events').where(firebase.firestore.FieldPath.documentId(), '>=', startStr).where(firebase.firestore.FieldPath.documentId(), '<=', endStr).get();
        const schedSnap = await getUserCol('schedules').where(firebase.firestore.FieldPath.documentId(), '>=', startStr).where(firebase.firestore.FieldPath.documentId(), '<=', endStr).get();
        const jourSnap = await getUserCol('journals').where(firebase.firestore.FieldPath.documentId(), '>=', startStr).where(firebase.firestore.FieldPath.documentId(), '<=', endStr).get();
    ...
    getMemoDataArray: async function() {
        const snap = await getUserCol('tasks').orderBy('createdAt').get();
        const rows = [["데이터분류", "ID", "내용/이름", "완료여부(O/X)", "라벨", "주소/URL", "생성일자(타임스탬프)"]];
        
        const linkDoc = await getUserCol('settings').doc('user_links').get();
        if (linkDoc.exists) {
    ...
    processScheduleRows: async function(rows, mode) {
        ...
        if (mode === 'overwrite') {
            ...
            for (const col of cols) {
                const snap = await getUserCol(col).where(firebase.firestore.FieldPath.documentId(), '>=', startStr).where(firebase.firestore.FieldPath.documentId(), '<=', endStr).get();
                snap.forEach(doc => {
        ...
        for (const dStr of sortedDates) {
            ...
            if (mode === 'merge') {
                const existDoc = await getUserCol('events').doc(dStr).get();
                if (existDoc.exists) {
            ...
            batch.set(getUserCol('events').doc(dStr), { eventList: newEventList, eventText: evText, updatedAt: Date.now() }, { merge: true });
            opCount++;

            batch.set(getUserCol('schedules').doc(dStr), { periods: scheduleDataMap[dStr], updatedAt: Date.now() }, { merge: true });
            opCount++;

            if (journalDataMap[dStr]) {
                let newJournalList = journalDataMap[dStr];
                if (mode === 'merge') {
                    const existDoc = await getUserCol('journals').doc(dStr).get();
                    if (existDoc.exists) {
                ...
                batch.set(getUserCol('journals').doc(dStr), { entries: newJournalList, updatedAt: Date.now() }, { merge: true });
                opCount++;
            }
        ...
        if (labelsChanged) {
            ...
            if (window.auth && window.auth.currentUser) {
                await getUserCol('settings').doc('labels').set({
    ...
    processMemoRows: async function(rows, mode) {
        ...
        if (mode === 'overwrite') {
            const snap = await getUserCol('tasks').get();
            snap.forEach(doc => {
                batch.delete(doc.ref);
                opCount++;
                if(opCount >= 400) { batchPromises.push(batch.commit()); batch = window.db.batch(); opCount = 0; }
            });
            await getUserCol('settings').doc('user_links').delete();
            if(opCount > 0) { batchPromises.push(batch.commit()); batch = window.db.batch(); opCount = 0; }
            await Promise.all(batchPromises);
            batchPromises.length = 0; 
        }
        ...
        if (newLinks.length > 0) {
            ...
            batch.set(getUserCol('settings').doc('user_links'), { links: newLinks }, { merge: true });
            opCount++;
        }
    ...
    getOrCreateSpreadsheet: async function(token) {
        const configRef = getUserCol('settings').doc('backup_config');
