...
import { getUserCol, getGroupCol, db, dbAPI } from '../firebase.js'; 
...
export const BackupManager = {
    ...
    // 🌟 현재 선택된 공간(개인/그룹)에 맞춰 동적으로 DB 경로를 찾아주는 핵심 함수
    getColRef: function(colName) {
        const scopeSelect = document.getElementById('backup-scope-select');
        const scope = scopeSelect ? scopeSelect.value : 'personal';
            ...
            <div>
                ...
                <div style="display:flex; flex-direction:column; gap:10px; background:#f8fafc; padding:15px; border-radius:8px; border:1px solid #e2e8f0;">
                    <select id="backup-period-select" onchange="window.BackupManager.onPeriodChange()" style="width:100%; padding:10px; border:1px solid #cbd5e1; border-radius:6px; font-size:1rem; outline:none; cursor:pointer; font-weight:bold; color:#334155;">
                        ...
                        <option value="all">전체 기간 (비워두기)</option>
                    </select>
    ...
    // 🌟 공간 선택 변경 이벤트: 그룹을 선택하면 구글 시트/캘린더 사용을 막고 CSV로 강제 변환
    ...
    onTargetChange: function() {
        ...
        if (target === 'calendar' && cCard && !cCard.style.cursor.includes('not-allowed')) {
            ...
            if(this.currentSpreadsheetId && sheetLink) sheetLink.style.display = 'block'; 
    ...
    openCurrentSheet: function() {
        if (this.currentSpreadsheetId) {
            window.open(`https://docs.google.com/spreadsheets/d/${this.currentSpreadsheetId}/edit`, '_blank');
        } else {
    ...
    executeExport: async function() {
        ...
        if (target === 'sheets') {
            if (scope !== 'personal') return alert("구글 시트 연동은 개인 데이터만 지원합니다.");
            await this.exportToSheets();
        } else if (target === 'csv') {
            await this.handleDownload();
        } else if (target === 'calendar') {
            if (scope !== 'personal') return alert("구글 캘린더 연동은 개인 데이터만 지원합니다.");
    ...
    onPeriodChange: function() {
        ...
        let sDate = new Date(d); let eDate = new Date(d);
        ...
        if (val === 'today') { } 
            ...
            sDate = new Date(y, m, 1); eDate = new Date(y, m + 1, 0);
        ...
        startInput.value = formatDate(sDate); endInput.value = formatDate(eDate);
    },
    ...
    getScheduleDataArray: async function() {
        ...
        const fetchCol = async (colName, useIdAsDate = true) => {
            ...
            if (val !== 'all' && startStr && endStr) {
                if (useIdAsDate) {
                    q = query(this.getColRef(colName), where(documentId(), '>=', startStr), where(documentId(), '<=', endStr));
                } else {
                    const startMs = new Date(startStr + 'T00:00:00').getTime();
                    const endMs = new Date(endStr + 'T23:59:59').getTime();
                    q = query(this.getColRef(colName), where('createdAt', '>=', startMs), where('createdAt', '<=', endMs));
                }
            } else {
                q = this.getColRef(colName);
            }
        ...
        for (const dStr of sortedDates) {
            ...
            if (incEvent && evMap[dStr] && evMap[dStr].eventList) {
                ...
                evMap[dStr].eventList.forEach(e => {
                    ...
                    metaList.push({ 
                        id: e.id, 
                        groupId: e.groupId || null, // 반복일정 식별자는 그대로 유지
                        sharedGroupId: isGroup ? scope : (e.sharedGroupId || null), // 공유그룹 식별자 분리
                ...
                if (needsDbUpdateEvent) {
                    dbAutoUpdates.push({
                        col: this.getColRef('events'),
                        docId: dStr,
                        data: { eventList: evMap[dStr].eventList, eventText: window.formatEventListToText ? window.formatEventListToText(evMap[dStr].eventList) : '', updatedAt: Date.now() }
                    });
                }
        ...
        if (dbAutoUpdates.length > 0) {
            window.ProgressModal.update("데이터 최적화 중...", 40);
    ...
    getMemoDataArray: async function() {
        ...
        let q = query(this.getColRef('tasks'), orderBy('createdAt'));
        if (val !== 'all' && startStr && endStr) {
            const startMs = new Date(startStr + 'T00:00:00').getTime();
            const endMs = new Date(endStr + 'T23:59:59').getTime();
            q = query(this.getColRef('tasks'), where('createdAt', '>=', startMs), where('createdAt', '<=', endMs), orderBy('createdAt'));
        }
    ...
    processScheduleRows: async function(rows, mode, matrixUpdates = []) {
        ...
        if (mode === 'overwrite') {
            ...
            for (const col of cols) {
                let q = (val === 'all') ? this.getColRef(col) : query(this.getColRef(col), where(documentId(), '>=', startStr), where(documentId(), '<=', endStr));
                const snap = await getDocs(q);
                snap.forEach(docSnap => {
        ...
        for (const dStr of sortedDates) {
            if (doEvent) {
                ...
                if (mode === 'merge') {
                    const existDoc = await getDoc(doc(this.getColRef('events'), dStr));
                ...
                batch.set(doc(this.getColRef('events'), dStr), { eventList: newEventList, eventText: evText, updatedAt: Date.now() }, { merge: true }); 
                opCount++;
            }

            if (doClass && scheduleDataMap[dStr]) {
                if (mode === 'merge') {
                    const scRef = doc(this.getColRef('schedules'), dStr);
                    ...
                    batch.set(doc(this.getColRef('schedules'), dStr), { periods: scheduleDataMap[dStr], updatedAt: Date.now() }, { merge: true }); 
                }
                opCount++;
            }
            ...
            if (doJournal && journalDataMap[dStr]) {
                ...
                if (mode === 'merge') {
                    const existDoc = await getDoc(doc(this.getColRef('journals'), dStr));
                    if (existDoc.exists()) newJournalList = getUniqueList([...(existDoc.data().entries || []), ...newJournalList]); 
                }
                batch.set(doc(this.getColRef('journals'), dStr), { entries: newJournalList, updatedAt: Date.now() }, { merge: true }); 
                opCount++;
            }
            ...
            if (doEval && evalDataMap[dStr] !== undefined) {
                ...
                if (mode === 'merge') {
                    const existDoc = await getDoc(doc(this.getColRef('evaluations'), dStr));
                    if (existDoc.exists()) {
                ...
                batch.set(doc(this.getColRef('evaluations'), dStr), { evalList: newEvalList, updatedAt: Date.now() }, { merge: true }); 
                opCount++;
            }

            if (opCount > 400) { batchPromises.push(batch.commit()); batch = writeBatch(db); opCount = 0; }
            processedCount++;
            if (window.ProgressModal) window.ProgressModal.update(`데이터베이스 저장 중... [${processedCount}/${totalDays}]`, 30 + (70 * (processedCount/totalDays)));
        }
    ...
    processMemoRows: async function(rows, mode) {
        ...
        if (mode === 'overwrite') {
            const snap = await getDocs(this.getColRef('tasks'));
            snap.forEach(docSnap => {
                batch.delete(docSnap.ref); opCount++;
                if(opCount >= 400) { batchPromises.push(batch.commit()); batch = writeBatch(db); opCount = 0; }
            });
        ...
        for (let i=1; i<rows.length; i++) {
            ...
            if (dataType === 'LINK' && !isGroup) {
                ...
                batch.set(doc(this.getColRef('tasks'), id), taskData, { merge: true });
                opCount++;
                if (opCount > 400) { batchPromises.push(batch.commit()); batch = writeBatch(db); opCount = 0; }
            }

            processedRows++;
            if (processedRows % 10 === 0 && window.ProgressModal) window.ProgressModal.update(`메모 데이터 저장 중... [${processedRows}/${totalRows}]`, 50 + (50 * (processedRows/totalRows)));
        }
    ...
    handleDownload: async function() {
        try {
            const scope = document.getElementById('backup-scope-select').value;
            const scopeName = scope === 'personal' ? '개인' : (this.myGroups.find(g => g.id === scope)?.name || '그룹');

            window.ProgressModal.show("로컬 파일(CSV) 통합 내보내기");
            window.ProgressModal.update(`[${scopeName}] 데이터 집계 및 통합 파일 생성 중...`, 30);
    ...
    handleUpload: async function(input) {
        ...
        const scope = document.getElementById('backup-scope-select').value;
        const scopeName = scope === 'personal' ? '개인 데이터' : '공유 그룹 데이터';
        ...
        try {
            window.ProgressModal.show(`로컬 통합 파일(CSV) 가져오기 [${scopeName}]`);
            ...
            window.ProgressModal.complete(`✅ [${scopeName}] 공간에 성공적으로 복원되었습니다!`, () => {
                if(window.BackupManager.modal) window.BackupManager.modal.close();
                store.hasUnsavedChanges = false; 
...
window.openGoogleSyncModal = () => { 
    window.BackupManager.openModal(); 
    setTimeout(() => { 
        const s = document.getElementById('backup-scope-select');
        if(s) { s.value = 'personal'; window.BackupManager.onScopeChange(); }
        const r = document.querySelector('input[name="backup-target"][value="calendar"]'); 
        if(r){ r.checked=true; window.BackupManager.onTargetChange(); } 
    }, 200); 
};
