...
import { store } from '../core/store.js'; // 🌟 전역 변수 에러 방지를 위해 명시적 임포트
import { formatDate, getEventLabels, getJournalLabels, getSemesterDates } from '../core/utils.js';
import { getUserCol, db } from '../firebase.js'; // 🌟 db 객체 명시적 임포트
...
export const BackupManager = {
    ...
    onPeriodChange: function() {
        ...
        // 🌟 전역 오류 방지: window.currentDate 대신 store.currentDate 사용
    ...
    getScheduleDataArray: async function() {
        ...
        const eventsSnap = await getDocs(query(getUserCol('events'), where(documentId(), '>=', startStr), where(documentId(), '<=', endStr)));
        const schedSnap = await getDocs(query(getUserCol('schedules'), where(documentId(), '>=', startStr), where(documentId(), '<=', endStr)));
        const jourSnap = await getDocs(query(getUserCol('journals'), where(documentId(), '>=', startStr), where(documentId(), '<=', endStr)));
        ...
        // 🌟 전역 오류 방지: window.periodNames 대신 store.periodNames 사용
...
// 🌟 [안전장치] 전역으로 안전하게 내보내어 HTML의 모든 onClick 이벤트가 작동하도록 연결
