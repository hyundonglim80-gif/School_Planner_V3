...
export const TimetableModule = {
  ...
  applyTimetableToCalendar: async function() {
    ...
    try {
      const eventSnap = await getUserCol('events').where(firebase.firestore.FieldPath.documentId(), '>=', sStr).get();
      const scheduleSnap = await getUserCol('schedules').where(firebase.firestore.FieldPath.documentId(), '>=', sStr).get();
      ...
      while (cur <= endObj) {
        ...
        if (dayIndex >= 1 && dayIndex <= 5) {
          ...
          if (dateStr <= eStr) { 
              ...
              const docRef = getUserCol('schedules').doc(dateStr);
  ...
  syncToCloud: async function() {
      ...
      try {
          await getUserCol('settings').doc('timetable_v5').set({
              semesterConfig: store.semesterConfig || {},
              templates: store.timetableTemplates || {},
              currentNames: store.periodNames || ["1", "2", "3", "4", "5", "6"],
              updatedAt: Date.now()
          });
      } catch (e) {
