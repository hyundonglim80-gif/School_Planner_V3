    ...
    exportToSheets: async function(options) {
        try {
            ...
            if (doSchedule) {
                ...
                const finalData = await BackupData.getScheduleDataArray(options);[cite: 1]
    ...
    importFromSheets: async function(options) {
        ...
        try {
            ...
            if (doSchedule) {
                ...
                await BackupData.processScheduleRows(rows, options, matrixUpdates);[cite: 1]
            }
            ...
            if (options.incMemo) {
                ...
                if(res.ok) {
                    const data = await res.json();
                    const rows = data.values || [];
                    await BackupData.processMemoRows(rows, options);[cite: 1]
                }
            }
