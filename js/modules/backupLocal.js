...
export const BackupLocal = {
    handleDownload: async function(options, scopeName) {
        try {
            ...
            if (doSchedule) {
                ...
                const data = await BackupData.getScheduleDataArray(options);[cite: 1]
            ...
            if (options.incMemo) {
                ...
                const memoData = await BackupData.getMemoDataArray(options);[cite: 1]
    ...
    handleUpload: async function(file, options, scopeName) {
        ...
        try {
            ...
            if (schedRows.length > 0) await BackupData.processScheduleRows(schedRows, options, matrixUpdates);[cite: 1]
            if (memoRows.length > 0) await BackupData.processMemoRows(memoRows, options);[cite: 1]
