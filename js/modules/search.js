...
export const SearchModule = {
  ...
  executeSearch: async function() {
    ...
    try {
        ...
        const scheduleSnap = await getUserCol('schedules').get();
        const journalSnap = await getUserCol('journals').get();
        const taskSnap = await getUserCol('tasks').get(); 
        ...
        try { const memoSnap = await getUserCol('memos').get(); memoSnap.forEach(doc => processMemoData(doc, doc.data())); } catch(e) {}
        ...
        if (matchedTasks.length > 0) {
          matchedTasks.forEach(task => {
            ...
            let taskCardHtml = `
              <div class="search-result-card" onclick="store.scope='memo'; window.ModalManager ? window.ModalManager.closeTop() : SearchModule.modalInstance.close(); window.render();" style="border-left: 5px solid #10b981;">
